...
This release introduces a major API upgrade from callbacks to Promises. Please see the [docs](https://github.com/ethereumjs/merkle-patricia-tree/tree/master/docs) for the latest method signatures.

Behind the scenes the library and its tests have been refactored to TypeScript. Use `import { BaseTrie as Trie } from 'merkle-patricia-tree'` for the base interface. In Ethereum applications stick with the Secure Trie Overlay `import { SecureTrie as Trie } from 'merkle-patricia-tree'`.
